...
subs = [1 2 3 7];%[1,2,3,7,8,9,10,11,12,14,15];%,16,17,18,19,20];
nSubs = length(subs);
...
ts = 15; % title size
fs = 9; % font size
lim = [0, 0.5]; % limits on color map and channel response axis
times = -500:4:1248;
...
for i = 1:nSubs
    ...
    t_ctf(i, :, :) = squeeze(mean(mean(tfs(:, :, :, :),1),3));
end


% average data across subjects, mirror the -180 channel at +180
tctf = squeeze(mean(t_ctf,1)); tctf = [tctf tctf(:,1)]; size(tctf)
max(tctf)
min(tctf)
% Setup plot axes
...
title('Forward Perceptron Model','FontSize',ts)
