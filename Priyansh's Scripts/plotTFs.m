...
subs = [3]%,2];%,3,7,8,9,10,11,12,14,15,16,17,18,19,20];
nSubs = length(subs);
...
for i = 1:nSubs
    ...
    t_ctf(i,:,:) = squeeze(mean(mean(em.tfs.total(fois,:,:,:,:),4),2)); % grab data, average across iterations (dimension 2) and test blocks (dimension 4). 
end
